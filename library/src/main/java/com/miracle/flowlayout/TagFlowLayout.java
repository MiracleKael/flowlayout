package com.miracle.flowlayout;
...
public class TagFlowLayout extends FlowLayout
    ...
    private int atLeastCheckedNum;
    private boolean isAtLeastMode;
    ...
    public TagFlowLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        atLeastCheckedNum = ta.getInt(R.styleable.TagFlowLayout_at_least_checked_num, -1);
        if (atLeastCheckedNum > 0) {
            isAtLeastMode = true;
        } else {
            isAtLeastMode = false;
        }
     ...
     */
    public void setAdapter(TagAdapter adapter, int defaultSelectedPos) {
        mTagAdapter = adapter;
        mTagAdapter.setOnDataChangedListener(this);
        mSelectedView.clear();
        mTagAdapter.setSelectedList(defaultSelectedPos);
    }
    ...
    private void doSelect(TagView child, int position) {
        if (!child.isChecked()) {
            ...
            if (isAtLeastMode && mSelectedView.size() == atLeastCheckedNum) {
                //如果是至少有N个选中模式 并且 当前已经选中的个数正好等于N
                return;
            }
